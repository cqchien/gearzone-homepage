...
import { reduceProductName, formatProductPrice } from '../../helpers';
...
function ProductView(props) {
  ...
  return (
    <Card
      ...
      loading={false}
      cover={
        <img className="max-w-100 max-h-100" src={avtUrl} alt="Product" />
      }
      hoverable>
      {/* Tên sản phẩm */}
      <div className="font-size-16px m-b-10">
        {reduceProductName(name)}
      </div>
      ...
      {quantity <= 5 && quantity > 0 && (
        <div className="Product-View-quantity font-size-14px">
      ...
      {/* Số lượng hàng còn, chỉ hiển thị khi còn ít hơn 5 */}
      {quantity === 0 && (
        <b className="Product-View-quantity font-size-16px">Đang hết hàng</b>
      )}
